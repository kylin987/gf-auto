...
通过 `/r/Conversation/getByCids` 确认当前店铺、买家和商品信息后再上报。双方历史会话的创建时间不作为拦截条件，
旧买家首次咨询新商品仍会触发。该事件按 `storeId + buyerId + itemId` 在本地持久化去重，
同一买家对同一商品只上报一次；超过 2 分钟的历史补发由 Outbox 丢弃。
...
任务类型：`task.xianyu.cancel_order`。仅在店铺开启秒退、订单已付款未发货且 SaaS 出票失败时下发。

客户端执行本地：

```text
POST http://127.0.0.1:8000/api/cancel_order
```

请求核心字段：

- `orderId`：闲鱼订单号。
- `closeReason`：卖家取消原因，当前默认“与买家协商一致”。
