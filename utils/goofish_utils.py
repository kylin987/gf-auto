import base64
import hashlib
import json
import os
import random
import shutil
import subprocess
import sys
import tempfile
import time
from functools import lru_cache
from pathlib import Path

def get_node_bin():
    """返回可用的 node 可执行文件路径，兼容 PyInstaller 打包后的目录结构。"""
    env_bin = os.environ.get('XY_NODE_BIN')
    if env_bin:
        return env_bin if os.path.isfile(env_bin) else None
    frozen = getattr(sys, '_MEIPASS', '')
    if frozen:
        candidates = [
            Path(frozen) / 'node' / 'node.exe',
            Path(frozen) / 'node' / 'node',
        ]
        for candidate in candidates:
            if candidate.exists():
                return str(candidate)
    return shutil.which('node')


def _load_static_js():
    frozen = getattr(sys, '_MEIPASS', '')
    candidates = []
    if frozen:
        candidates.append(Path(frozen) / 'static' / 'goofish_js_version_2.js')
    candidates.extend([
        Path(__file__).resolve().parent.parent / 'static' / 'goofish_js_version_2.js',
        Path.cwd() / 'static' / 'goofish_js_version_2.js',
    ])
    for candidate in candidates:
        if candidate.exists():
            return candidate.read_text(encoding='utf-8')
    raise FileNotFoundError('未找到 static/goofish_js_version_2.js')


node_bin = get_node_bin()
if node_bin:
    node_dir = str(Path(node_bin).parent)
    if node_dir not in ('.', '') and node_dir not in os.environ.get('PATH', ''):
        os.environ['PATH'] = node_dir + os.pathsep + os.environ.get('PATH', '')


@lru_cache(maxsize=1)
def _static_js_source():
    return _load_static_js()


def _run_js(expression, raw=False):
    """用 Node 直接执行 JS 表达式，避免外部 JS 运行时库在 Windows 上的兼容问题。"""
    if not get_node_bin():
        raise RuntimeError('未检测到 Node.js，请先安装 Node.js 18+，或在打包前把 node 放入 node_bin/')
    program = _static_js_source() + f'\n;console.log({expression});\n'
    fd, path = tempfile.mkstemp(suffix='.js', prefix='xianyu_js_')
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            f.write(program)
        result = subprocess.run(
            [get_node_bin(), path],
            capture_output=True,
            text=True,
            timeout=30,
            creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0,
        )
    finally:
        try:
            os.unlink(path)
        except OSError:
            pass
    if result.returncode != 0:
        raise RuntimeError(f'JS 执行失败: {result.stderr.strip() or result.stdout.strip()}')
    output = result.stdout.strip()
    if not output:
        raise RuntimeError('JS 执行无输出')
    return output if raw else json.loads(output.splitlines()[-1])


def trans_cookies(cookies_str):
    cookies = dict()
    for i in cookies_str.split("; "):
        try:
            cookies[i.split('=')[0]] = '='.join(i.split('=')[1:])
        except:
            continue
    return cookies

def trans_cookies_str(cookies_dict):
    cookies_str = ''
    for key, value in cookies_dict.items():
        cookies_str += f"{key}={value}; "
    return cookies_str[:-2]

def get_session_cookies(session):
    cookies = session.cookies.get_dict()
    return cookies

def get_session_cookies_str(session):
    cookies = session.cookies.get_dict()
    cookies_str = ''
    for key, value in cookies.items():
        cookies_str += f"{key}={value}; "
    return cookies_str[:-2]

def generate_mid():
    return f'{int(1000 * random.random())}{int(time.time() * 1000)} 0'

def generate_uuid():
    return f'-{int(time.time() * 1000)}1'

def generate_device_id(user_id):
    chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
    parts = []
    for i in range(36):
        if i in (8, 13, 18, 23):
            parts.append('-')
        elif i == 14:
            parts.append('4')
        elif i == 19:
            parts.append(random.choice('89ab'))
        else:
            parts.append(random.choice(chars))
    return ''.join(parts) + '-' + str(user_id)

def generate_sign(t, token, data):
    arg = f'{token}&{t}&34839810&{data}'
    return hashlib.md5(arg.encode('utf-8')).hexdigest()

def decrypt(data):
    return _run_js(f'decrypt({json.dumps(data)})', raw=True)

if __name__ == '__main__':
    # t = 1741667630548
    # token = 'b7e897bf9767618a32b439c6103fe1cb'
    # data = '{"appKey":"444e9908a51d1cb236a27862abc769c9","deviceId":"ED4CBA2C-5DA0-4154-A902-BF5CB52409E2-3888777108"}'
    # print(generate_sign(t, token, data))
    msg = ""
    msg = ""

    res = decrypt(msg)
    print(res)
